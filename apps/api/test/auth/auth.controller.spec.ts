...
import type { AuthCookieService } from '../../src/modules/auth/services/auth-cookie.service';
...
function harness() {
  const auth = {
    ...
    startGoogleLogin: vi.fn().mockResolvedValue({
      state: 'oauth-state',
...
describe('AuthController', () => {
  ...
  it('sets the OAuth state cookie before redirecting to Google', async () => {
    ...
    expect(auth.startGoogleLogin).toHaveBeenCalledWith('/app');
  ...
  it('still clears the one-time state when callback validation fails', async () => {
    ...
    auth.completeGoogleLogin.mockRejectedValueOnce(new UnauthorizedException('invalid state'));
  ...
  it('consumes a provider-declared OAuth error and redirects without exposing its description', async () => {
    ...
    expect(auth.completeGoogleLoginError).toHaveBeenCalledWith('query-state', 'cookie-state');
