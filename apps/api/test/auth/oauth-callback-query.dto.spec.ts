...
import { OauthCallbackQueryDto } from '../../src/modules/auth/dto/oauth-callback-query.dto';
...
describe('OauthCallbackQueryDto', () => {
  ...
  it('continues to reject callback parameters outside the explicit Google contract', async () => {
  ...
  it('rejects a callback that claims a different authorization server issuer', async () => {
    const errors = await validateCallback({
      ...googleCallback,
      iss: 'https://accounts.attacker.example',
    });
