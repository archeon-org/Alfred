...
describe('GoogleOidcService', () => {
  ...
  it('exchanges the code and validates issuer payload, nonce, email and optional domain', async () => {
    ...
    await expect(
      service.verifyCallback('authorization-code', {
        codeVerifier: 'verifier',
        nonce: 'expected-nonce',
      }),
    ).resolves.toEqual({
  ...
  it('rejects missing tokens, nonce mismatches and foreign Workspace domains', async () => {
    ...
    await expect(
      service.verifyCallback('code', { codeVerifier: 'verifier', nonce: 'nonce' }),
    ...
    await expect(
      service.verifyCallback('code', { codeVerifier: 'verifier', nonce: 'nonce' }),
    ).rejects.toThrow('Google identity validation failed');
    ...
    await expect(
      service.verifyCallback('code', { codeVerifier: 'verifier', nonce: 'nonce' }),
    ).rejects.toThrow('Google Workspace domain is not allowed');
  });
});
