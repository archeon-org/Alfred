...
export interface GoogleAuthorizationChallenge {
...
export class GoogleOidcService {
  ...
  async createChallenge(): Promise<GoogleAuthorizationChallenge> {
  ...
  createAuthorizationUrl(state: string, challenge: GoogleAuthorizationChallenge): string {
    const client = this.requireClient();
  ...
  async exchangeCode(code: string, codeVerifier: string, nonce: string): Promise<VerifiedIdentity> {
    const client = this.requireClient();
