 ...
 * runtime title agent. Without the agent bridge the message waits in the chat as a draft.
 */
  ...
  const start = useCallback(
    async (text: string): Promise<boolean> => {
      ...
      try {
        ...
        const sent = bridgeAvailable && session.send(conversation.id, draft);
        void navigate(
          `/app/conversations/${conversation.id}`,
          sent ? undefined : { state: { draft } },
        );
        return true;
      } catch {
