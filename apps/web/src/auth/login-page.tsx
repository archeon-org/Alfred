...
import { getGoogleLoginUrl, safeReturnTo } from './auth-api';
          ...
          <div className="mt-7" aria-live="polite">
            ...
            !featureFlags.flags.googleOAuth ? (
              <div className="rounded-xl border border-line bg-canvas px-4 py-3 text-sm leading-6 text-muted">
                Google OAuth est désactivé sur cet environnement. Aucun autre fournisseur de
                connexion n’est encore configuré.
              </div>
            ) : null}

            {status === 'anonymous' && featureFlags.status === 'error' ? (
              <div className="space-y-3" role="alert">
                <p className="text-sm leading-6 text-danger-700">
                ...
                <button
                  className={cn(buttonVariants({ variant: 'outline' }), 'w-full')}
                  onClick={() => void featureFlags.reload()}
                  type="button"
                >
                  Réessayer
                </button>
