...
try {
  ...
  console.error(
    "Missing dependencies. Please run: yarn add -D inquirer@^8.0.0 chalk@^4.1.2 semver",
  );
  process.exit(1);
}
...
async function release() {
  ...
  try {
    ...
    originalVersionConfig = JSON.parse(
    ...
    console.log(
      chalk.cyan(
        `Current Version: ${currentVersion} (Build: iOS ${originalVersionConfig.iosBuildNumber}, Android ${originalVersionConfig.androidVersionCode})`,
      ),
    );
    ...
    const answers = await inquirer.prompt([
      {
        ...
        choices: [
          {
            name: `Patch (${semver.inc(currentVersion, "patch")}) - Bug fixes`,
            value: "patch",
          },
          {
            name: `Minor (${semver.inc(currentVersion, "minor")}) - New features`,
            value: "minor",
          },
          {
            name: `Major (${semver.inc(currentVersion, "major")}) - Breaking changes`,
            value: "major",
          },
    ...
    if (shouldBuild && platform) {
      ...
      if (platform === "android") {
        ...
        console.log(
          chalk.blue(
            `Building for Android - bumping Android version code only`,
          ),
        );
      } else if (platform === "ios") {
        newIosBuildNumber = (
          parseInt(originalVersionConfig.iosBuildNumber, 10) + 1
        ).toString();
        console.log(
          chalk.blue(`Building for iOS - bumping iOS build number only`),
        );
      }
    } else {
    ...
    console.log(
      chalk.green(
        `✔ Build numbers: Android ${newAndroidVersionCode}, iOS ${newIosBuildNumber}`,
      ),
    );
 ...
 ${answers.description}\n`;
    ...
    if (currentChangelog.startsWith(headerMarker)) {
      newChangelog = currentChangelog.replace(
        headerMarker,
        headerMarker + changelogEntry,
      );
    } else {
      newChangelog =
        headerMarker +
    ...
    if (shouldBuild && platform) {
      ...
      try {
        execSync(
          `eas build --platform ${platform} --profile production --local`,
          { stdio: "inherit" },
        );
        ...
        if (originalVersionConfig) {
          fs.writeFileSync(
            versionFilePath,
            JSON.stringify(originalVersionConfig, null, 2) + "\n",
          );
        }
    ...
    try {
      ...
      execSync(
        `git commit -m "chore(release): ${newVersion} - ${answers.description}"`,
      );
      ...
      console.error(
        chalk.yellow(
          "⚠ Files were modified but not committed. You may want to commit manually or revert.",
        ),
      );
    }
