...
function bumpVersion() {
  try {
    ...
    if (platform === "android") {
      ...
      console.log(
        `Bumped Android version code to: ${versionConfig.androidVersionCode}`,
      );
    } else if (platform === "ios") {
      versionConfig.iosBuildNumber = (
        parseInt(versionConfig.iosBuildNumber, 10) + 1
      ).toString();
      bumpedPlatform = `iOS ${versionConfig.iosBuildNumber}`;
      console.log(
        `Bumped iOS build number to: ${versionConfig.iosBuildNumber}`,
      );
    } else {
      ...
      console.log(
        `Bumped both platforms: Android ${versionConfig.androidVersionCode}, iOS ${versionConfig.iosBuildNumber}`,
      );
    }
