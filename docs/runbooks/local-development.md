...
After applying all pending migrations, including `1789500000000-create-artifacts`, set
`FEATURE_FILE_UPLOADS_ENABLED=true` in the environment used by the API and restart it. The flag
enables the composer paperclip, the « Fichiers » tab of the right panel and the library at
`/app/files`; with the flag off the API hides every `/api/files` route and the browser shows none
of them. Accepted files are PDF, DOCX and PNG/JPEG/WebP/GIF images, decided from their bytes.

Where bytes go is chosen once at startup, and the API refuses to start if it cannot write there:

- **No `FILE_STORAGE_S3_*` value:** files are kept under `FILE_STORAGE_LOCAL_ROOT`
  (`var/uploads`, relative to the directory the API starts in — `apps/api` with `pnpm dev:api`).
  This is a development profile, refused when `NODE_ENV=production`, and it cannot be used by the
  Compose API container, whose filesystem is read-only.
- **A described bucket** (`FILE_STORAGE_S3_BUCKET` and `FILE_STORAGE_S3_REGION`, plus an endpoint
  for anything but AWS): any S3-compatible provider. The key pair is optional; without it the AWS
  SDK resolves an IAM role. A partial description fails at startup rather than falling back to the
  disk. The principal needs `PutObject`, `GetObject` and `DeleteObject` on the objects and
  `ListBucket` on the bucket.

| Provider      | `FILE_STORAGE_S3_ENDPOINT`                                      | `FILE_STORAGE_S3_REGION` | Notes                                              |
| ------------- | --------------------------------------------------------------- | ------------------------ | -------------------------------------------------- |
| AWS S3        | empty                                                           | the bucket's region      | an IAM role is preferred to static keys            |
| MinIO         | `http://localhost:9000` (host) or `http://minio:9000` (Compose) | `us-east-1`              | path-style addressing, the default here            |
| Cloudflare R2 | `https://<account>.r2.cloudflarestorage.com`                    | `auto`                   | one write per second on a given key                |
| IBM COS       | `https://s3.<region>.cloud-object-storage.appdomain.cloud`      | any, e.g. `eu-de`        | HMAC service credentials                           |
| Scaleway      | `https://s3.fr-par.scw.cloud`                                   | `fr-par`                 | the region must match the endpoint                 |
| OVHcloud      | `https://s3.gra.io.cloud.ovh.net`                               | `gra`                    | rejects SDK checksum headers, already switched off |

For a local S3-compatible server, set `FILE_STORAGE_S3_SECRET_ACCESS_KEY` (at least eight
characters), `FILE_STORAGE_S3_BUCKET=alfred-files`, `FILE_STORAGE_S3_REGION=us-east-1`,
`FILE_STORAGE_S3_ACCESS_KEY_ID=alfred-api` and the endpoint above in `.env`, then run
`docker compose --profile files up -d minio minio-bucket`. The bucket step creates the bucket and
a user limited to it; the MinIO administrator (`MINIO_ROOT_*`, generated by `pnpm setup:env`) is
never given to the API. MinIO's community edition is archived and unmaintained: it is a
development stand-in, and a real provider is qualified before go-live by pointing
`TEST_FILE_STORAGE_S3_ENDPOINT`, `_BUCKET`, `_ACCESS_KEY_ID`, `_SECRET_ACCESS_KEY` and optionally
`_REGION` at it and running `pnpm --filter @alfred/api test`, which then runs the content-store
contract against it.

`FILE_UPLOAD_MAX_BYTES` defaults to 5242880 and `FILE_QUOTA_BYTES_PER_USER` to 26214400; only
original bytes count, and nothing expires: deleting a file is what frees space. Exceeding the
quota returns `quota_exceeded` with the used, reserved and limit figures. Text extraction and
image reduction run in a worker thread after the upload answers, bounded by
`FILE_EXTRACTION_TIMEOUT_MS`, `FILE_EXTRACTION_MAX_PDF_PAGES` and `FILE_EXTRACTED_TEXT_MAX_CHARS`;
a scanned PDF without a text layer is reported as unreadable, since there is no OCR. What a
message sends to the agent is bounded by `FILE_PROMPT_TOKENS_PER_DOCUMENT` (10000) and
`FILE_PROMPT_TOKENS_PER_EXECUTION` (30000). Migration rollback refuses to drop the tables while
stored content exists. See [ADR 0027](../adr/0027-uploaded-file-content-store.md) and
[ADR 0028](../adr/0028-personal-file-library.md).
...
docker compose --env-file .env.example --profile files config --quiet
```
